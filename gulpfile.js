...
gulp.task( 'zip', ['build'], function() {
	...
	return gulp.src( './' )
		.pipe( exec( 'cd ./../; rm -rf pixfields.zip; cd ./build/; zip -r -X ./../pixfields.zip ./pixfields; cd ./../; rm -rf build' ) );
...
gulp.task( 'copy-folder', function() {
	...
	return gulp.src( './' )
		.pipe( exec( 'rm -Rf ./../build; mkdir -p ./../build/pixfields; cp -Rf ./* ./../build/pixfields/' ) );
} );
...
gulp.task( 'build', ['copy-folder'], function() {
	...
	files_to_remove.forEach( function( e, k ) {
		files_to_remove[k] = '../build/pixfields/' + e;
	} );
