<?php
/**
 * Template used to display the pixfields gallery
 * Available vars:
...
?>
<div id="pixfields_gallery" class="gallery  gallery-columns-<?php echo $columns; ?>  cf  js-pixfields-gallery">
	<?php
	$idx = 1;
	foreach ( $attachments as $attachment ) {
		if ( 'selected' == self::get_attachment_class( $attachment ) ) {
			$select_label = __( 'Deselect', 'pixfields_l10n' );
		} else {
			$select_label = __( 'Select', 'pixfields_l10n' );
		}
								...
								<ul class="actions-nav  nav  nav--stacked">
									<li>
										<a class="meta__action  zoom-action" href="<?php echo $image_full[0]; ?>" data-photoid="<?php echo $image_id_tag; ?>">
											<span class="button-text"><?php _e( 'Zoom', 'pixfields_l10n' ); ?></span>
										</a>
									</li>
									<li>
										<hr class="separator"/>
